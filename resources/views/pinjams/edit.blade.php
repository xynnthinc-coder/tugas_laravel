...
@section('page-sub', 'Perbarui data peminjaman dan status pengembalian')
...
<div class="max-w-3xl">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
        <div class="px-8 py-6 border-b border-gray-100">
            <h3 class="text-xl font-semibold text-slate-900 mb-1">Perbarui Peminjaman</h3>
            <p class="text-sm text-slate-500">Periksa kembali status agar stok buku tetap akurat.</p>
        </div>
        <div class="p-8">
            <form method="POST" action="{{ route('pinjam.update', $pinjam->id) }}" class="space-y-6">
                @csrf
                @method('PUT')
                @php
                    $selectedBukuIds = old('buku_ids');
                    if (!is_array($selectedBukuIds)) {
                        $selectedBukuIds = $pinjam->pinjamDetails->pluck('buku_id')->all();
                    }
                @endphp
                ...
                <div>
                    ...
                    <select name="siswa_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        <option value="{{ $siswa->id }}" {{ old('siswa_id', $pinjam->siswa_id) == $siswa->id ? 'selected' : '' }}>
                            {{ $siswa->nama }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="buku_ids[]" multiple required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        @php
                            $isSelected = in_array($buku->id, $selectedBukuIds);
                            $isDisabled = !$isSelected && ($buku->stok < 1);
                        @endphp
                        <option value="{{ $buku->id }}" {{ $isSelected ? 'selected' : '' }} {{ $isDisabled ? 'disabled' : '' }}>
                            {{ $buku->judul_buku }} (stok: {{ $buku->stok }}){{ $isDisabled ? ' - stok habis' : '' }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="petugas_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        <option value="{{ $petugas->id }}" {{ old('petugas_id', $pinjam->petugas_id) == $petugas->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="date" name="tanggal_pinjam" value="{{ old('tanggal_pinjam', $pinjam->tanggal_pinjam) }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_kembali" value="{{ old('tanggal_kembali', $pinjam->tanggal_kembali) }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <!-- Status otomatis (dipinjam). Pengembalian dilakukan via tombol "Kembalikan". -->
